...
|---|---|
| AC already exists in `docs/ACs.org` | Delivery evidence even when its archived ticket records no test result. Preserve the AC and reconcile any missing provenance or test relationship. |
...
| Explicit passing test evidence | Delivered AC. Reconcile it into `docs/ACs.org` with its original traceability. |
...
| RT is present in the maintained harness and the single suite run passed | Current passing evidence. Correct its legacy pending or unverified RT and AC status in `docs/ACs.org`. |
...
For the reverse checksum, verify every maintained RT against `docs/ACs.org` and
