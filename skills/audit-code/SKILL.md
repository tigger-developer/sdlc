...
6. **Summarize.** Allocate a unique report directory with `mktemp -d` and write `audit-code-<NNN>.md` there: each finding with file paths and line numbers, categorized as blockers (must fix) or smells (worth discussing), with a specific recommended fix for each. Include the documentation-impact validation and its evidence. Render and open it if a renderer is available: use `AGENT_HTML_RENDERER` when set; otherwise use `pandhtml` if present. If neither exists, report the temporary markdown path. Retain the report only for the active review and remove the exact directory through explicit teardown afterwards. **In chat: one line stating blocker count, smell count, and the report path.**
